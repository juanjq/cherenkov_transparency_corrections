...
python_script="$root_scripts""script_1_scaling.py"
...
# ##################################################################
# # Step 0: Submitting a job to compute all the IRFs if do not exist
...
# merge_job_id=$(sbatch -p short --output="$path_output""slurm_merge-%j.out" $merge_script | awk '{print $4}')
